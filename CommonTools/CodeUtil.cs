...
namespace CommonTools
{
    public class CodeUtil
    {
        ...
        public static string GenCode(string hookType, string templateStr, string className, string functionName, string param, List<ParaItem> checkedItems, int paramCount, int workId)
        {
            string reStr = "";
            ...
            for (int i = 2; i < checkedItems.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (checkedItems[i].MemPrint)
                    {
                        ...
                        paramStr += string.Format(s, i - 2, checkedItems[i].MemLength);

                        s = "\n\t\t\t" + "console.log({0});\n"
                            + "\t\t\t" + "console.log(Memory.readByteArray({0}, {1}));\n";
                        if (leavePara != "")
                            s = "\t\t\t" + s;
                        leavePara += string.Format(s, arg, checkedItems[i].MemLength);
                    }
                    if (checkedItems[i].MemU64Print)
                    {
                        s = "\t\t\t" + "var p{0} = ptr(ptr(parseInt(args[{0}])).readU64());\n"
                            + "\t\t\t" + "console.log(Memory.readByteArray(p{0}, {1})); \n";
                        paramStr += string.Format(s, i - 2, checkedItems[i].Mem64Length);
                    }
                }
            }
